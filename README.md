...
**使用范围：仅限个人非商业学习与实验、招聘评审。未经书面许可，不得用于商业项目、收费服务或商业产品。** 具体条款见 [LICENSE](LICENSE)。
...
`Library`、`Temp`、`Logs`、`obj` 为生成的缓存、日志和编译输出。
...
本项目采用自定义的[学习与评审使用许可证](LICENSE)。允许为个人学习下载、运行和修改，也允许招聘方进行代码审阅和试玩评估；分享或 Fork 时须保留署名、来源与许可条款，修改版同样受用途限制。

这是用途受限的源码公开项目，不属于开源许可证授权的项目。许可变更前已按 MIT 发布的历史版本，仍保留其原有授权；本次变更不追溯收回已经授予的权利。
